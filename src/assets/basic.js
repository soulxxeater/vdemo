...
var onPKIServiceNotify = function(event, arg, ec){
...
window.onload=function(){
    ...
    //初始化，传入回调函数，在初始化成功后才调用其它接口
    gPKISvc.init(PROTO_AUTO, onPKIServiceNotify);
};
...
function UpdateSetSupportDevFlags()
{
	...
	mainForm.dev_flags.value = setVal;
}
...
function Test_Verify()
{
    ...
    var signed_data = mainForm.SignedData.value;
...
function SetSignMessageTSPURL()
{
	...
	var ts_url = mainForm.signmsg_ts_url.value;
	if(mainForm.signmsg_with_ts.checked && ts_url.length == 0){
		alert('时间戳地址不能为空！');
		return;
	}
...
function Test_Encrypt()
{
    ...
    var res = gPKISvc.SOF_EncryptData(mainForm.enc_cert.value, sEncryptPlainData, function(res){
		t2 = new Date().getTime();
